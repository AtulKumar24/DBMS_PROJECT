...
import 'dart:io'; // NEW: Added for date formatting (HttpDate)

// ... (Home and _HomeState classes remain exactly the same) ...
...
class _PolicyholdersDialogState extends State<PolicyholdersDialog> {
  ...
  void _showAddOrEditDialog({Map<String, dynamic>? existingPolicyholder}) {
    final bool isEditing = existingPolicyholder != null;

    if (isEditing) {
      _nameController.text = existingPolicyholder['name'];

      // UPDATED: Format the date string
      try {
        // Parse the GMT date string
        final DateTime parsedDate = HttpDate.parse(existingPolicyholder['dob']);
        // Format it as YYYY-MM-DD
        _dobController.text = "${parsedDate.year.toString().padLeft(4, '0')}-"
            "${parsedDate.month.toString().padLeft(2, '0')}-"
            "${parsedDate.day.toString().padLeft(2, '0')}";
      } catch (e) {
        // Fallback if parsing fails (e.g., it's already YYYY-MM-DD)
        _dobController.text = existingPolicyholder['dob'];
      }

      _contactController.text = existingPolicyholder['contact'];
    } else {
        ...
        title: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                color: (isEditing ? Colors.blue : Colors.green).withOpacity(0.1),
                borderRadius: BorderRadius.circular(8),
              ),
              child: Icon(
            ...
            Text(isEditing ? 'Edit Policyholder' : 'Add Policyholder'),
          ],
        ),
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (isEditing) {
                _updatePolicyholder(existingPolicyholder['policyholder_id']);
              } else {
                _addPolicyholder();
              }
            },
  ...
  // ... (_addPolicyholder, _updatePolicyholder, _confirmDelete, _deletePolicyholder are unchanged) ...
          ...
          SnackBar(
            content: const Row(
              children: [
                Icon(Icons.check_circle, color: Colors.white),
                SizedBox(width: 10),
                Text('Policyholder updated successfully!'),
              ],
            ),
            backgroundColor: Colors.blue, // Changed color for update
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(10),
            ),
...
class _PoliciesDialogState extends State<PoliciesDialog> {
  ...
  void _showAddOrEditDialog({Map<String, dynamic>? existingPolicy}) {
    final bool isEditing = existingPolicy != null;

    if (isEditing) {
      _selectedPolicyholderId = existingPolicy['policyholder_id'];
      _premiumController.text = existingPolicy['premium'];
      _policyTypeController.text = existingPolicy['policy_type'];
    } else {
          ...
          title: Row(
            children: [
              Container(
                ...
                child: Icon(
                  isEditing ? Icons.edit_document : Icons.add_business,
                  color: isEditing ? Colors.blue : Colors.green,
                ),
              ),
              const SizedBox(width: 12),
              Text(isEditing ? 'Edit Policy' : 'Add Policy'),
            ],
          ),
          content: SingleChildScrollView(
            child: Column(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.pop(context);
                if (isEditing) {
                  _updatePolicy(existingPolicy['policy_id']);
                } else {
                  _addPolicy();
                }
              },
  ...
  Future<void> _updatePolicy(String id) async {
    if (_selectedPolicyholderId == null ||
        _premiumController.text.isEmpty ||
        _policyTypeController.text.isEmpty) {
      setState(() {
    ...
    try {
      await widget.apiService.updatePolicy(
        id: id,
        policyholderId: _selectedPolicyholderId!,
        premium: _premiumController.text,
        policyType: _policyTypeController.text,
...
class _ClaimsDialogState extends State<ClaimsDialog> {
  ...
  void _showAddOrEditDialog({Map<String, dynamic>? existingClaim}) {
    final bool isEditing = existingClaim != null;

    if (isEditing) {
      _selectedPolicyId = existingClaim['policy_id'];

      // UPDATED: Format the date string
      try {
        final DateTime parsedDate = HttpDate.parse(existingClaim['claim_date']);
        _claimDateController.text =
        "${parsedDate.year.toString().padLeft(4, '0')}-"
            "${parsedDate.month.toString().padLeft(2, '0')}-"
            "${parsedDate.day.toString().padLeft(2, '0')}";
      } catch (e) {
        _claimDateController.text = existingClaim['claim_date'];
      }

      _claimAmountController.text = existingClaim['claim_amount'];
      _statusController.text = existingClaim['status'];
    } else {
          ...
          title: Row(
            children: [
              Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(
                  color: (isEditing ? Colors.blue : Colors.orange).withOpacity(0.1),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: Icon(
                  isEditing ? Icons.edit_note : Icons.add_task,
                  color: isEditing ? Colors.blue : Colors.orange,
                ),
              ),
              const SizedBox(width: 12),
              Text(isEditing ? 'Edit Claim' : 'Add Claim'),
            ],
          ),
          content: SingleChildScrollView(
            child: Column(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.pop(context);
                if (isEditing) {
                  _updateClaim(existingClaim['claim_id']);
                } else {
                  _addClaim();
                }
              },
  ...
  // ... (_addClaim, _updateClaim, _confirmDelete, _deleteClaim are unchanged) ...
  ...
  Future<void> _updateClaim(String id) async {
    if (_selectedPolicyId == null ||
        _claimDateController.text.isEmpty ||
        _claimAmountController.text.isEmpty ||
        _statusController.text.isEmpty) {
    ...
    try {
      await widget.apiService.updateClaim(
        id: id,
        policyId: _selectedPolicyId!,
        claimDate: _claimDateController.text,
        claimAmount: _claimAmountController.text,
...
class _AgentsDialogState extends State<AgentsDialog> {
  ...
  void _showAddOrEditDialog({Map<String, dynamic>? existingAgent}) {
    final bool isEditing = existingAgent != null;

    if (isEditing) {
      _agentNameController.text = existingAgent['agent_name'];
      _phoneController.text = existingAgent['phone'];
    } else {
        ...
        title: Row(
          children: [
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(
                color: (isEditing ? Colors.blue : Colors.purple).withOpacity(0.1),
                borderRadius: BorderRadius.circular(8),
              ),
              child: Icon(
                isEditing ? Icons.edit : Icons.person_add,
                color: isEditing ? Colors.blue : Colors.purple,
              ),
            ),
            const SizedBox(width: 12),
            Text(isEditing ? 'Edit Agent' : 'Add Agent'),
          ],
        ),
        content: SingleChildScrollView(
          child: Column(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(context);
              if (isEditing) {
                _updateAgent(existingAgent['agent_id']);
              } else {
                _addAgent();
              }
            },
  ...
  Future<void> _updateAgent(String id) async {
    ...
    try {
      await widget.apiService.updateAgent(
        id: id,
        agentName: _agentNameController.text,
        phone: _phoneController.text,
      );
